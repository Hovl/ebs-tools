...
public final class CurrencyConverter {
    ...
    public Float convert(Float amount, String fromCurrency, String toCurrency)
        ...
        try {
            amount = (float) convert((double) amount, fromCurrency, toCurrency);
    ...
    public Float convert(Float amount, String fromCurrency, String toCurrency, boolean cute)
            throws IOException, ParseException {
        if (amount == null) return null;

        if (!cute) return convert(amount, fromCurrency, toCurrency);

        amount = convert(amount, fromCurrency, toCurrency);

        if (amount == null) return null;

        amount = (Math.round(amount*2F)/2F - 0.01F);

        return amount > 0 ? amount : 0.25F;
    }
